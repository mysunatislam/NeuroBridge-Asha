...
const API_BASE = (import.meta.env.VITE_API_URL ?? "http://localhost:8000/v1").replace(/\/$/, "");
...
async function startRemoteSession(profile: FingerSpeakProfile, profileId: string, telemetrySalt: string = crypto.randomUUID()): Promise<RemoteLink> {
